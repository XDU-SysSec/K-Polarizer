...
def parse_offset_pairs(offset_file):
    ...
    with open(offset_file) as f:
        for line in f:
            ...
            start_off = int(start, 16) - 0x40000000
            end_off = int(end, 16) - 0x40000000
            ranges.append((start_off, end_off))
    return ranges

orig_file = 'libc-2.27.so.transformed.trans2'
slimmed_file = 'libc.so.transformed_slimmed'
...
with open(slimmed_file, 'rb+') as f:
    ...
    for start, end in offset_ranges:
        func_content = original_text_content[start:end]
        mmapped_file[text_start_offset + start:text_start_offset + end] = func_content
